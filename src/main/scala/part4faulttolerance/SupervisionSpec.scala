...
class SupervisionSpec extends TestKit(ActorSystem("SupervisionSpec"))
  ...
  "A supervisor" should {
    "resume its child in case of a minor fault" in {
      ...
      child ! "Akka is awesome because something and something and else"
      child ! Report
      expectMsg(3)
    }
...
object SupervisionSpec {
  ...
  class Supervisor extends Actor {
    ...
    override val supervisorStrategy: SupervisorStrategy = OneForOneStrategy() {
      ...
      case _: IllegalAccessException => Stop
  ...
  class FussyWordCounter extends Actor {
    ...
    override def receive: Receive = {
      ...
      case sentence: String =>
        if (sentence.length > 20) throw new RuntimeException("Sentence is to long")
        else if (!Character.isUpperCase(sentence(0))) throw new IllegalAccessException("Sentence must start with uppercase")
